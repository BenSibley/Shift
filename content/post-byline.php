...
if ( $author_display == 'hide' ) {
	printf( _x( 'Published %s', 'This blog post was published on some date', 'shift' ), $date );
} elseif ( $date_display == 'hide' ) {
	printf( _x( 'Published by %s', 'This blog post was published by some author', 'shift' ), $author );
} else {
	printf( _x( 'Published %1$s by %2$s', 'This blog post was published on some date by some author', 'shift' ), $date, $author );
}
echo '</div>';
