...
function ct_shift_register_widget_areas() {
	...
	register_sidebar( array(
		'name'          => __( 'Primary Sidebar', 'shift' ),
		'id'            => 'primary',
		'description'   => __( 'Widgets in this area will be shown in the sidebar next to the main post content', 'shift' ),
	...
	function ct_shift_update_fields( $fields ) {
		...
		$fields['author'] =
			'<p class="comment-form-author">
	            <label for="author">' . __( "Name", "shift" ) . $label . '</label>
	            <input id="author" name="author" type="text" placeholder="' . __( "Jane Doe", "shift" ) . '" value="' . esc_attr( $commenter['comment_author'] ) .
			'" size="30" ' . $aria_req . ' />
	        </p>';

		$fields['email'] =
			'<p class="comment-form-email">
	            <label for="email">' . __( "Email", "shift" ) . $label . '</label>
	            <input id="email" name="email" type="email" placeholder="' . __( "name@email.com", "shift" ) . '" value="' . esc_attr( $commenter['comment_author_email'] ) .
			'" size="30" ' . $aria_req . ' />
	        </p>';

		$fields['url'] =
			'<p class="comment-form-url">
	            <label for="url">' . __( "Website", "shift" ) . '</label>
		...
		if ( ( $show_full_post == 'yes' ) && ! is_search() ) {
			if ( $ismore ) {
				...
				if ( ! empty( $read_more_text ) ) {
					...
					the_content( __( 'Continue reading', 'shift' ) . " <span class='screen-reader-text'>" . get_the_title() . "</span>" );
				}
			} else {
				the_content();
			}
...
if ( ! function_exists( 'ct_shift_excerpt_read_more_link' ) ) {
	function ct_shift_excerpt_read_more_link( $output ) {
		...
		if ( ! empty( $read_more_text ) ) {
			...
			return $output . "<p><a class='more-link' href='" . esc_url( get_permalink() ) . "'>" . __( 'Continue reading', 'shift' ) . " <span class='screen-reader-text'>" . get_the_title() . "</span></a></p>";
		}
	}
}
add_filter( 'the_excerpt', 'ct_shift_excerpt_read_more_link' );
...
if ( ! function_exists( 'ct_shift_new_excerpt_more' ) ) {
	function ct_shift_new_excerpt_more( $more ) {
...
add_filter( 'excerpt_more', 'ct_shift_new_excerpt_more' );
...
if ( ! function_exists( '_wp_render_title_tag' ) ) :
	function ct_shift_add_title_tag() {
		?>
		<title><?php wp_title( ' | ' ); ?></title>
		<?php
	}
	add_action( 'wp_head', 'ct_shift_add_title_tag' );
endif;
