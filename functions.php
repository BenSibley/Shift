		...
		if ( ( $show_full_post == 'yes' ) && ! is_search() ) {
			if ( $ismore ) {
				...
				if ( ! empty( $read_more_text ) ) {
					...
					the_content( esc_html__( 'Continue reading', 'shift' ) . " <span class='screen-reader-text'>" . esc_html( get_the_title() ) . "</span>" );
				}
			} else {
				the_content();
			}
...
if ( ! function_exists( 'ct_shift_excerpt_read_more_link' ) ) {
	function ct_shift_excerpt_read_more_link( $output ) {
		...
		if ( ! empty( $read_more_text ) ) {
			...
			return $output . "<p><a class='more-link' href='" . esc_url( get_permalink() ) . "'>" . esc_html__( 'Continue reading', 'shift' ) . " <span class='screen-reader-text'>" . esc_html( get_the_title() ) . "</span></a></p>";
		}
	}
}
